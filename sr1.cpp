...
typedef void(__cdecl* HeliT)(void);
HeliT Heli = (HeliT)0x82793f30;
...
bool enable_camera_first_mode = false;
...
bool grab_IP = true;
bool player_is_host = true;
bool AUTO_JOIN_PLUGINS = true;
std::string gamertag = "srclient"; 
std::string own_ip_address = ""; 
IXAudio2* pXAudio2Engine = NULL;
HANDLE KeyHandle;
HANDLE PimpHandle;
//SOCKET currentSock;
HANDLE moduleHandle2;
HANDLE moduleHandleP;
DWORD PthreadId;
Detour<void> menu_state_Detour;
Detour<void> am_i_server_Detour;
Detour<void> s_hkConvexVerticesShapeFourVectors_Detour;
Detour<ULONGLONG>Pimp_Detour;
Detour<void> FUN_82714b00_Detour;
string xuidgkI64u;
string xuidgkI64o;
string xuidgkI64X;
string fixedgamertag;
bool show_commands_sent  = false;
int* idP = new int(1336);
bool car = false;
bool wait_for_events = true;
//Detour<unsigned long long> cmds_Detour;
void DT_menu_state(int r3) {
	DbgPrint("Function called %i\n", r3);
	//menu_state_Detour.CallOriginal(r3);
	//r3 = 1176764512;
}
void DT_am_i_server(void) {
	DbgPrint("Function called\n");

	//r3 = 1176764512;
}
//s_hkConvexVerticesShapeFourVectors_Detour.SetupDetour(0x82799170, hkConvexVerticesShapeFourVectors);
void hkConvexVerticesShapeFourVectors() {
	DbgPrint("hkConvexVerticesShapeFourVectors Function called\n");
	//s_hkConvexVerticesShapeFourVectors_Detour.CallOriginal(r3);
	//r3 = 1176764512;
}

void FUN_82714b00(int r3,int r4) { 
	//DbgPrint("FUN_82714b00 function called %i %i %i %i\n");
	//return FUN_82714b00_Detour.TakeDownDetour();
}


//unsigned long long cmds(const char r3[]) { 
//	 DbgPrint("command called %i\n");
//	 return cmds_Detour.CallOriginal();
//return cmds_Detour.CallOriginal();
...
void DoThis() {
	DbgPrint("did it");
	ConsoleCommand("level_ambient 0 255 0");
}
...
DWORD XamUserGetNameHook( DWORD dwUserIndex, LPSTR pUserName, DWORD cchUserName)
{
	...
	strcpy(pUserName, "xCoolCanadianx");

	return ret;
}


void Pimp(unsigned long long *r3,unsigned long *r4) {
	if (*r4 == 5177414) {
	sp.sendCar();
	}
	Pimp_Detour.CallOriginal(r3,r4);
}

//	Pimp_Detour.TakeDownDetour();
...
void xui(PLDR_DATA_TABLE_ENTRY ModuleHandle){  
	while (true)
	{
		...
		else if (XamGetCurrentTitleId() == 0x545107d1) // Saints Row
		{
			...
			{
				*(uint32_t*)(0x8265F694) = 0x48000AA4;
				*(uint32_t*)(0x82660134) = 0x480B8EF9;
				*(uint32_t*)(0x82660268) = 0x480B8E14;
				menu_state_Detour.SetupDetour(0x827336a0, DT_menu_state);
				Pimp_Detour.SetupDetour(0x825344B0, Pimp);
				//Function_8263CA30_Detour.SetupDetour(0x8263CA30,Function_8263CA30);
				//cmds_Detour.SetupDetour(0x8263CB10, cmds);
				...
				*/
				...
				//0x82799170
				s_hkConvexVerticesShapeFourVectors_Detour.SetupDetour(0x82799170, hkConvexVerticesShapeFourVectors);
				am_i_server_Detour.SetupDetour(0x8235cf30, DT_am_i_server);
				//FUN_82714b00_Detour.SetupDetour(0x82714b00, FUN_82714b00);
				//827336a0
				//82732ff0
				//0x82732ff0 = freezes
				XUID xuid;
				char buf3[16];
				XUserGetXUID(0, &xuid);
				sprintf(buf3, "%I64u",xuid);
				//sprintf(buf4, "%I64o",xuid);
				//sprintf(buf5, "%I64x",xuid);
				xuidgkI64u = string(buf3);
				//xuidgkI64o = string(buf4);
				//xuidgkI64X = string(buf5);
				//replace(fixedgamertag.begin(), fixedgamertag.end(), ' ', '_');
				sp.start();
				int z1 = 0;
				gamertag = "";
				while (z1 < 30) { 
					gamertag += reinterpret_cast<char*>(0xC2A43611 + z1);
					z1++;
				}
				DbgPrint("hit!!!!");
				fixedgamertag = gamertag;
				sp.Connect2Servers();
				bbInitialized = true;
			}
			else  {
